...
class Reservation extends Component {
  ...
  renderDayHours(date, item, hour) {
    const today = date && isToday(date) ? this.style.today : undefined;
    const dayNames = getDefaultLocale().dayNamesShort;
    return (
      <View style={this.style.day} testID={RESERVATION_DATE}>
        <Text allowFontScaling={false} style={[this.style.hourNum, today]}>
          {hour?.hour}
        </Text>
        <Text allowFontScaling={false} style={[this.style.hourText, today]}>
          {hour?.postFix}
        </Text>
      </View>
    );
  ...
  render() {
    const {item, date, hour, content} = this.props;
    // let content;
    // if (item) {
    ...
    return (
      <View style={this.style.container}>
        ...
        {/* {this.renderDayHours(date, item, hour)} */}
